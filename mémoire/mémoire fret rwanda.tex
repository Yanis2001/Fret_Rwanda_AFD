...
\begin{document}
	...
	\begin{mdframed}[frametitle=Encadré 1 : illustration des 4 étapes pour un réseau fictif]
...
\end{equation}
	...
	Les paramètres du modèle ne sont pas tous également bien connus. Trois familles reposent sur du dire d'expert ou sur des ordres de grandeur empruntés à la littérature plutôt que sur une mesure directe~: les élasticités $\beta_s$ de la fonction de friction (section~\ref{subsubsec:friction}), les taux de conversion monnaie-tonne qui traduisent les grandeurs de la matrice de comptabilité sociale en tonnages, et la valeur du temps qui pondère la composante temporelle du coût généralisé (équation~\ref{eq:cout_temps}). Ce sont ces trois familles que nous soumettons à variation.
	
	\paragraph{Plan d'expérience.} Faire varier un paramètre à la fois, toutes choses égales par ailleurs, ne teste qu'une seule direction de l'espace des hypothèses et ne dit rien des compensations entre paramètres. Nous retenons donc un plan par \emph{hypercube latin}~: chaque axe du plan est découpé en autant d'intervalles équiprobables qu'il y a de tirages, et chaque intervalle n'est visité qu'une fois, ce qui couvre l'espace de façon homogène avec un petit nombre d'exécutions. Le plan compte dix-sept axes — un par élasticité sectorielle, un par valeur au tonne sectorielle, et un axe unique pour la valeur du temps, appliqué simultanément aux trois véhicules types — pour vingt tirages. Chaque axe est tiré sous la forme d'un multiplicateur de la valeur de référence, dans un intervalle de $\pm 30\%$. Cette amplitude est volontairement large~: il ne s'agit pas de refléter une incertitude statistique mesurée, dont nous ne disposons pas, mais de vérifier que les conclusions du modèle survivent à une révision substantielle de ses hypothèses les moins assurées. Chaque tirage donne lieu à une réexécution complète des modules de coûts, de distribution et d'affectation~; la géographie du réseau, qui ne dépend d'aucun de ces paramètres, est réutilisée telle quelle.
	
	\paragraph{Lecture des résultats.} Les tirages étant indépendants par construction, l'influence d'un paramètre sur un indicateur de sortie se lit directement dans la corrélation de rang de Spearman entre le multiplicateur tiré et l'indicateur obtenu. Nous présentons d'abord la dispersion d'ensemble des indicateurs agrégés autour du scénario de référence, puis l'attribution de cette dispersion aux différents paramètres, famille par famille.
